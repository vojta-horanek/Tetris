...
public:
    PixelInspector(int width, int height, int bpp) : pixels_(width * height * bpp, 0), width_(width), height_(height),
                                                     bpp_(bpp) {
    }
...
BEGIN_TEST(int, char*
...

        {
            ...
            EXPECT_TRUE(pixels.Test3x3(19, 19, 0x260, 255, 128, 0),
                        "depends on OpenGL implementation, which may or may not draw last pixel of a line", NON_FATAL);
...

        {
            ...
            Texture texture(renderer, TESTDATA_DIR
            "/crate.png");
