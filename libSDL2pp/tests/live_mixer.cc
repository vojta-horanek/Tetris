...
BEGIN_TEST(int, char*
        ...
        {
            // Init
            EXPECT_EQUAL(mixerlib.GetInitFlags(), MIX_INIT_OGG, "Mix_Init(0) doesn't return mask of init'd loaders",
                         NON_FATAL);

            EXPECT_TRUE(mixerlib.InitMore(MIX_INIT_FLAC) & MIX_INIT_FLAC);

            EXPECT_EQUAL(mixerlib.GetInitFlags(), MIX_INIT_OGG | MIX_INIT_FLAC,
                         "Mix_Init(0) doesn't return mask of init'd loaders", NON_FATAL);
        }

        Mixer mixer(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, MIX_DEFAULT_CHANNELS, 4096);

        Chunk sound(TESTDATA_DIR
        "/test.ogg");
        Music music(TESTDATA_DIR
        "/test.ogg");
