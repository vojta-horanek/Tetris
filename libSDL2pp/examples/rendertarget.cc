...
static const unsigned char pixels[4 * 4 * 4] = {
        RGBA(0xff, 0x00, 0x00, 0xff), RGBA(0xff, 0x80, 0x00, 0xff), RGBA(0xff, 0xff, 0x00, 0xff),
        RGBA(0x80, 0xff, 0x00, 0xff),
        RGBA(0xff, 0x00, 0x80, 0xff), RGBA(0xff, 0xff, 0xff, 0xff), RGBA(0x00, 0x00, 0x00, 0x00),
        RGBA(0x00, 0xff, 0x00, 0xff),
        RGBA(0xff, 0x00, 0xff, 0xff), RGBA(0x00, 0x00, 0x00, 0x00), RGBA(0x00, 0x00, 0x00, 0xff),
        RGBA(0x00, 0xff, 0x80, 0xff),
        RGBA(0x80, 0x00, 0xff, 0xff), RGBA(0x00, 0x00, 0xff, 0xff), RGBA(0x00, 0x80, 0xff, 0xff),
...
int main(int, char *[]) try {
    ...
    Window window("libSDL2pp demo: sprites", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, MY_SCREEN_WIDTH,
    ...
    Texture target1(render, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_TARGET, MY_RENDERTARGET_SIZE,
                    MY_RENDERTARGET_SIZE);
    target1.SetBlendMode(SDL_BLENDMODE_BLEND);

    Texture target2(render, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_TARGET, MY_RENDERTARGET_SIZE,
                    MY_RENDERTARGET_SIZE);
    ...
    while (1) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            render.Clear();
            render.Copy(target1, NullOpt, Rect(0, 0, MY_RENDERTARGET_SIZE / 2, MY_RENDERTARGET_SIZE / 2),
                        SDL_GetTicks() / 10000.0 * 360.0);
            render.Copy(target1, NullOpt,
                        Rect(MY_RENDERTARGET_SIZE / 2, 0, MY_RENDERTARGET_SIZE / 2, MY_RENDERTARGET_SIZE / 2),
                        SDL_GetTicks() / 10000.0 * 360.0);
            render.Copy(target1, NullOpt,
                        Rect(0, MY_RENDERTARGET_SIZE / 2, MY_RENDERTARGET_SIZE / 2, MY_RENDERTARGET_SIZE / 2),
                        SDL_GetTicks() / 10000.0 * 360.0);
            render.Copy(target1, NullOpt,
                        Rect(MY_RENDERTARGET_SIZE / 2, MY_RENDERTARGET_SIZE / 2, MY_RENDERTARGET_SIZE / 2,
                             MY_RENDERTARGET_SIZE / 2), SDL_GetTicks() / 10000.0 * 360.0);
        ...
        render.Copy(target1, NullOpt,
                    Rect((MY_SCREEN_WIDTH - MY_SCREEN_HEIGHT) / 2, 0, MY_SCREEN_HEIGHT, MY_SCREEN_HEIGHT),
                    SDL_GetTicks() / 10000.0 * 360.0);
